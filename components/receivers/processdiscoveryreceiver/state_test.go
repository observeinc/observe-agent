...
func TestProcessStateLifecycle(t *testing.T) {
	...
	events, _ := state.apply(ScanResult{Processes: []ProcessSnapshot{java}, Complete: true}, 2, 10, 1, 0)
	...
	events, _ = state.apply(ScanResult{Processes: []ProcessSnapshot{changed}, Complete: true}, 2, 10, 1, 0)
	require.Len(t, events, 1)
	assert.Equal(t, "process.changed", events[0].Name)
	state.apply(ScanResult{Complete: true}, 2, 10, 1, 0)
	events, _ = state.apply(ScanResult{Complete: true}, 2, 10, 1, 0)
	require.Len(t, events, 1)
...
func TestIncompleteScanDoesNotStopProcess(t *testing.T) {
	state := newProcessState()
	state.apply(ScanResult{Processes: []ProcessSnapshot{snapshot(10, 100, "java")}, Complete: true}, 1, 10, 1, 0)
	events, _ := state.apply(ScanResult{Complete: false}, 1, 10, 1, 0)
	assert.Empty(t, events)
}
...
func TestProcessReappearanceResetsTerminationMisses(t *testing.T) {
	...
	state.apply(ScanResult{Processes: []ProcessSnapshot{process}, Complete: true}, 2, 10, 1, 0)
	state.apply(ScanResult{Complete: true}, 2, 10, 1, 0)
	state.apply(ScanResult{Processes: []ProcessSnapshot{process}, Complete: true}, 2, 10, 1, 0)
...
func TestProcessLimitRetainsTrackedProcesses(t *testing.T) {
	...
	state.apply(ScanResult{Processes: []ProcessSnapshot{tracked}, Complete: true}, 1, 1, 1, 0)

	events, saturated := state.apply(ScanResult{Processes: []ProcessSnapshot{
		snapshot(1, 100, "python"), tracked,
	}, Complete: true}, 1, 1, 1, 0)
...
func TestOldGenerationExitCannotDeleteReusedPID(t *testing.T) {
	...
	events, _ := state.apply(ScanResult{Processes: []ProcessSnapshot{snapshot(10, 200, "java")}, Complete: true}, 1, 10, 1, 0)
	assert.Empty(t, events)
...
func TestProcessStateEmitsPeriodicSnapshot(t *testing.T) {
	...
	state.apply(ScanResult{Processes: []ProcessSnapshot{process}, Complete: true}, 1, 10, 1, time.Minute)
	process.ObservedAt = time.Unix(161, 0)
	events, _ := state.apply(ScanResult{Processes: []ProcessSnapshot{process}, Complete: true}, 1, 10, 1, time.Minute)
	require.Len(t, events, 1)
